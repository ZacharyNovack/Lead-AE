...
idx2root = {
    ix: root for root, ix in root2idx.items()
}
idx2root[0] = None

idx2qual = {
    ix: qual for qual, ix in qual2idx.items()
}
idx2qual[0] = None

def qual_and_root_to_pitches(qual, root):
    if qual == 'NC' or root == 'NC':
        return []
    if qual == 'M':
        ...
        raise ValueError(f'Unknown chord quality: {qual}')


def get_pitches_from_chord(chord_root, chord_qual):
    real_root = idx2root[chord_root]
    real_qual = idx2qual[chord_qual]
    if real_root in [None, 'NC'] or real_qual is [None, 'NC']:
        return []
    return qual_and_root_to_pitches(real_qual, real_root)
...
def get_encoding(bt_tok=False):
    """Return the encoding configurations."""
    if bt_tok:
        loc_type_code_map = TYPE_CODE_MAP.copy()
        loc_type_code_map["beat"] = 5
        loc_code_type_map = utils.inverse_dict(loc_type_code_map)
        loc_n_tokens = N_TOKENS.copy()
        loc_n_tokens[0] += 1
    else:
        loc_type_code_map = TYPE_CODE_MAP
        loc_code_type_map = CODE_TYPE_MAP
        loc_n_tokens = N_TOKENS
    return {
        "resolution": RESOLUTION,
        "max_beat": MAX_BEAT,
        "max_duration": MAX_DURATION,
        "dimensions": DIMENSIONS,
        "n_tokens": loc_n_tokens,
        "type_code_map": loc_type_code_map,
        ...
        "code_type_map": loc_code_type_map,
...
def encode_notes(notes, encoding, bt_tok=None):
    ...
    for beat, position, pitch, duration, program in notes:
        ...
        # if new onset, append beat marker to codes
        if bt_tok == 'onset' and (codes[-1][beat_dim] != row[beat_dim] or codes[-1][position_dim] != row[position_dim]):
            codes.append([type_code_map["beat"], row[beat_dim], row[position_dim], 0, 0, 0])
        elif bt_tok == 'beat' and codes[-1][beat_dim] != row[beat_dim]:
            codes.append([type_code_map["beat"], row[beat_dim], 1, 0, 0, 0])
        elif bt_tok == 'bar4' and codes[-1][beat_dim] // 4 != row[beat_dim] // 4:
            codes.append([type_code_map["beat"], row[beat_dim] // 4, 0, 0, 0, 0])
        elif bt_tok == 'bar2' and codes[-1][beat_dim] // 2 != row[beat_dim] // 2:
            codes.append([type_code_map["beat"], row[beat_dim] // 2, 0, 0, 0, 0])

        row[pitch_dim] = pitch_code_map[pitch]
        row[duration_dim] = duration_code_map[min(duration, max_duration)]
        row[instrument_dim] = instrument_code_map[instrument]
        # if codes[-1][beat_dim] != row[beat_dim] or codes[-1][position_dim] != row[position_dim]:
        #     pass
            # codes.append([type_code_map["note"], row[beat_dim], row[position_dim], 0, row[duration_dim], row[instrument_dim]])
        if row not in codes[-len(instrument_codes):]:
            codes.append(row)
...
def decode_notes(codes, encoding, ohc=None, dec_chords=None):
    ...
    for ix, row in enumerate(codes):
        ...
        if event_type in ("start-of-song", "instrument", "start-of-notes"):
            ...
            # if row[instrument_dim] == 0: # new beat
                # continue
        ...
        elif event_type == "beat" and dec_chords is not None: # need to convert chord into notes
            beat = code_beat_map[int(row[beat_dim])]
            position = code_position_map[int(row[position_dim])]
            pitches = get_pitches_from_chord(int(dec_chords[str(beat)][0]), int(dec_chords[str(beat)][1]))
            if len(pitches) == 0:
                continue
            for pitch in pitches:
                notes.append((beat, position, pitch, 12, 4))
        else:
...
def decode(codes, encoding, ohc=None, dec_chords=None):
    ...
    # Decode codes into a note sequence
    notes = decode_notes(codes, encoding, ohc=ohc, dec_chords=dec_chords)

    # Reconstruct the music object
    music = reconstruct(notes, resolution)
    # print(music.tracks[0].notes)
